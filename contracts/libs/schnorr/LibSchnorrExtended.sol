...
library LibSchnorrExtended {
        ...
        returns (uint, address)
    {
        ...
        LibSecp256k1.Point[] memory pubKeys = new LibSecp256k1.Point[](privKeys.length);
        ...
        LibSecp256k1.Point[] memory noncePubKeys = new LibSecp256k1.Point[](privKeys.length);
        for (uint i; i < privKeys.length; i++) {
    ...
    {
        // k = H(x ‖ m) mod Q
        return uint(keccak256(abi.encodePacked(privKey, message))) % LibSecp256k1.Q();
    }
