...
contract Subnet is OwnableUpgradeable {
    ...
    function initialize(address tokenAddress, address xTokenAddress, address _sentryContract, address _superNodeContract) public initializer {
        tokenContract = IERC20(tokenAddress);
        xTokenContract = IERC20(xTokenAddress);
        minStakable = 5000 * 10**18;
        __Ownable_init(msg.sender);
        sentryContract = ISentryContract(_sentryContract);
        superNodeContract = ISentryContract(_superNodeContract);
    }
    ...
    function hashRewardData(RewardClaimData[] calldata claimData) public pure  returns(bytes32 hash) {
        // uint len = claimData[0].subnetId.length;
        for (uint i; i < claimData.length; i++) {
            if (i == 0) {
                 hash = keccak256(abi.encodePacked(bytes6(claimData[i].subnetId), claimData[i].amount));
            } else {
                hash = keccak256(abi.encodePacked(hash, bytes6(claimData[i].subnetId), claimData[i].amount));
            }
        }
        return hash;
    }
    ...
    function getMinSignerCount(uint cycleNumLicences) public pure returns(uint) {
    ...
    function verifyClaim(Claim calldata claim) public view returns(bool, bytes32) {
    ...
    function rewardValidator(
        Claim calldata claim
            ...
            
           {
            ...
            uint licenceCount = sentryContract.getCycleLicenseCount(claim.cycle);
           ...
           //  uint startLicence = ((hash/salt) % licenceCount) + 1000;
            ...
            xTokenContract.operatorMint(sentryContract.operatorsOwner(claim.validator), validatorBaseReward+(claim.totalCost/2));
            for (uint i = 0; i<30; i++) {
                uint decodedLicence = MLUtils.lcg( (((uint(claimHash)/((uint(claimHash) % 1000)) % licenceCount) + 1000) + (i * ((uint(claimHash) % 1000) + 1))) % licenceCount) + 1000;
                bytes memory signer = LibSecp256k1Extended.pubKeyFromPoints(claim.signers[signerIndex]);
                bytes memory owner = sentryContract.licenseOperator(decodedLicence);
                if (keccak256(signer) == keccak256(owner)) {
                    validCount++;
                    signerIndex++;
                    // TODO mint/transfer xToken to the license owner
                    xTokenContract.operatorMint(sentryContract.licenseOwner(decodedLicence), sentryBaseReward+(claim.totalCost/(2*claim.signers.length)));
                }
                if (validCount >= (claim.signers.length - 1) || validCount >= minSigners*2) {
                    break;
                }
            }
